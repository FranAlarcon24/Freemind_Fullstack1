...
public class ActividadController {
    ...
    @PatchMapping("/{id}")
    public ResponseEntity<Actividad> patchActividad(@PathVariable Integer id, @RequestBody Actividad actividad) {
        Actividad patchedActividad = actividadService.patchActividad(id, actividad);
        if (patchedActividad == null) {
            return ResponseEntity.notFound().build();
        }
        return ResponseEntity.ok(patchedActividad);
    }
